...
class ActionMannedRetreat(Action):
    ...
    # 后撤参数（基于导弹参数：导弹最大射程72km，必须后撤到射程外）
    RETREAT_DISTANCE_KM = 40        # 后撤到中心后方距离 km（保持一定前压）
    SAFE_DISTANCE_KM = 75           # 与最近敌机的安全距离 km（导弹射程72km + 安全余量）
    ...
    # 紧急逃跑参数（基于导弹射程72km）
    EMERGENCY_DISTANCE_KM = 50      # 紧急逃跑触发距离 km（敌机进入50km时紧急后撤）
    ...
    def tick(self, agent) -> str:
        ...
        for i, unit in enumerate(manned):
            if is_emergency:
                # 紧急逃跑：直接向后方逃离（必须逃到导弹射程72km外）
                escape_lon = center_lon - OfficialParams.km_to_deg(self.SAFE_DISTANCE_KM + 10) * enemy_dir
