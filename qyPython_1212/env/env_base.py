...
import utilities.yxScriptTreeFunc as yxFunc
...
class EnvBase:
    def __init__(self):
        ...
        if self.run_times > 0:
            self.rest_run_time = self.run_times
        ...
        for task in task_list:
            if task['taskId'] == config.task_id:
                if 'positions' in task:
                    ...
                    yxFunc.battlefield = self.battlefield
    ...
    def get_done(self):
        ...
        for side_item in raw_observation['side_list']:
            ...
            for track_item in side_item['track_list']:
                if track_item['platform_entity_type'] == "导弹":
                    if track_item['target_name'] not in missile_name_list:
                        missile_name_list.append(track_item['target_name'])
        missile_num += len(missile_name_list)
        if self.funTool.sim_data.wait_missile_num > 0:
            missile_num += self.funTool.sim_data.wait_missile_num
        ...
        if raw_observation['header']['sim_time'] > 60:
            ...
            if red_manned_num == 0 and blue_manned_num == 0:
                done[0] = True
            elif red_manned_num == 0:
                done[0] = True
                done[2] = True
            elif blue_manned_num == 0:
                done[0] = True
                done[1] = True
            elif missile_num == 0 or raw_observation['header']['sim_time'] > 15 * 60 - 1:
                done[0] = True
