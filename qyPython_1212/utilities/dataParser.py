...
class Track:#目标属性
    def __init__(self, data = None, track_platform = None):
        self.target_id = None
...
class SimData:
    def __init__(self):
        ...
        self.frame = 0
        self.user_side = 'red'
        self.missile_data = {'red': [], 'blue': []}
        self.wait_missile_num = 0
    ...
    def add_or_update_platform(self, data):
        ...
        if 'type' in data:
            if data['type'] == '导弹':
                mis_data={
                    'is_fired_list': [],
                    'engage_target': data['track_list'][0]['platform'],
                    ...
                    "pitch": data['pitch'],
                    "platform_entity_side": data['side'],
                    "platform_entity_type": data['type'],
                    "roll": data['roll'],
                    "speed": data['speed'],
                    "target_id": data['id'],
                ...
                self.missile_data[data['side']].append(mis_data)
                self.wait_missile_num -= 1
    ...
    def remove_platform(self, data):
        if 'name' in data:
            ...
            for ik, iv in self.track_dict.items():
                if len(iv["is_fired_list"]) > 0:
                    if name in iv["is_fired_list"]:
                        self.track_dict[ik]["is_fired_list"].remove(name)
            for key, value in self.track_info.items():
                if name in value:
                    self.track_info[key].pop(name)
                for nk, tnl in value.items():
                    if name in tnl:
                        self.track_info[key][nk].pop(name)
                    # break
            self.remove_from_track_dict(name)
            red_data = self.missile_data['red']

            for n in red_data:
                if n['target_name'] == name:
                    self.missile_data['red'].remove(n)
                    return
            blue_data = self.missile_data['blue']
            for n in blue_data:
                if n['target_name'] == name:
                    self.missile_data['blue'].remove(n)
                    return
    ...
    def update_weapon_quantity(self, data):
        if 'launchPlatformName' in data:
            ...
            if name in self.platform_info:
                self.wait_missile_num += 1
                if isinstance(self.platform_info[name].weapons, dict):
                    weapon = self.platform_info[name].weapons['导弹']
                    if isinstance(weapon, dict):
                        self.platform_info[name].weapons['导弹']['quantity'] -= 1
                    elif isinstance(weapon, object):
                        self.platform_info[name].weapons['导弹'].quantity -= 1
                elif isinstance(self.platform_info[name].weapons, list):
                    weapon = self.platform_info[name].weapons[0]
                    if isinstance(weapon, dict):
    ...
    def remove_track(self, data):
        if 'targetPlatformName' in data:
            ...
            if target_name in self.platform_info:
                target_platform = self.platform_info[target_name]
            self_platform = self.platform_info[name]
            if self_platform.side == 'red':
                red_team_track = self.track_info['red']
                if target_platform is not None:
                    if target_platform.side == 'red' and target_platform.type == '导弹':
                        return
                ...
                if target_platform is not None:
                    if target_platform.side == 'blue' and target_platform.type == '导弹':
                        return
                        ...
                        for own_name, track_dic in value.items():
                            if len(track_dic) > 0:
                                for track_name, track in track_dic.items():
                                    ...
                                    if len(self.missile_data['red']) > 0:
                                        for d in self.missile_data['red']:
                                            if d['target_id'] not in track_id_list:
                                                red_team['track_list'].append(d)
                                                track_id_list.append(d['target_id'])
                            ...
                            if len(track_dic) > 0:
                                for track_name, track in track_dic.items():
                                    if track_name in self.track_dict:
                                        ...
                                        track.is_fired_num = 0
                                        track.engage_target = None
                                    if len(self.missile_data['blue']) > 0:
                                        for d in self.missile_data['blue']:
                                            if d['target_id'] not in track_id_list:
                                                blue_team['track_list'].append(d)
                                                track_id_list.append(d['target_id'])
                                    if track.target_id not in track_id_list:
                                        track_id_list.append(track.target_id)
