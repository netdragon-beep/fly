...
class SmartFireControl:
    ...
    # === 飞机性能参数（根据仿真平台设定）===
    # 有人机
    MANNED_MAX_SPEED = 600         # 有人机最大速度 m/s (约2马赫)
    MANNED_CRUISE_SPEED = 400      # 有人机巡航速度 m/s

    # 无人机
    UAV_MAX_SPEED = 500            # 无人机最大速度 m/s (约1.5马赫)
    UAV_CRUISE_SPEED = 350         # 无人机巡航速度 m/s

    # === 导弹参数 ===
    # 有人机导弹 (中程空空导弹)
    MANNED_MISSILE_SPEED = 1200    # 导弹平均速度 m/s (约3.5马赫)
    MANNED_MISSILE_RANGE = 20000   # 导弹最大射程 20km
    MANNED_MISSILE_DURATION = 25   # 导弹最大飞行时间 秒

    # 无人机导弹 (近程空空导弹)
    UAV_MISSILE_SPEED = 1000       # 导弹平均速度 m/s (约3马赫)
    UAV_MISSILE_RANGE = 15000      # 导弹最大射程 15km
    UAV_MISSILE_DURATION = 20      # 导弹最大飞行时间 秒

    # === 武器参数（保留兼容）===
    MANNED_MAX_RANGE = 20000       # 最大射程 20km
    MANNED_NEZ_HEAD_ON = 12000     # 迎头NEZ 12km (基准值)
    MANNED_NEZ_TAIL = 6000         # 尾追NEZ 6km (基准值)
    MANNED_OPTIMAL_RANGE = 15000   # 最优射程 15km

    UAV_MAX_RANGE = 15000          # 最大射程 15km
    UAV_NEZ_HEAD_ON = 8000         # 迎头NEZ 8km (基准值)
    UAV_NEZ_TAIL = 4000            # 尾追NEZ 4km (基准值)
