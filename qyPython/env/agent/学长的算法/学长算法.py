...
class ActionProtectMannedVision(Action):
    """无弹药无人机围绕有人机巡逻，补齐有人机视野"""
    def tick(self, agent) -> str:
        # 找出未被占用且已打光弹药的无人机
        protectors = []
        for unit in agent.own_units:
            if unit.get('type') != '无人机':
                continue
            if unit['name'] in agent.commanded_units:
                continue
            has_ammo = False
        ...
        # 保护中心：有人机平均位置，没有则回到战场中心
        manned = [u for u in agent.own_units if u.get('type') == '有人机']
        if manned:
            center_lat = sum(m.get('latitude', agent.center_lat) for m in manned) / len(manned)
            center_lon = sum(m.get('longitude', agent.center_lon) for m in manned) / len(manned)
        else:
        ...
        for idx, uav in enumerate(protectors):
            # 将保护机布置在有人机后方120°扇区
            base_angle = 180
            spread = 120
            if num > 1:
                unit_angle = base_angle - spread/2 + (idx / (num - 1)) * spread
            else:
            ...
            target_lat = center_lat + radius * math.cos(angle_rad)
            target_lon = center_lon + radius * math.sin(angle_rad)
            target_pt = (target_lat, target_lon, 3600)
            agent.add_action(decCmd.fly_to_point(uav['name'], target_pt, 360), uav['name'])
...
class BTDemoAgent_v1(AutoAgentBase):
    def __init__(self, side, name):
        ...
        self.bt_root = Selector([
            ...
            Sequence([
                ...
                ActionProtectMannedVision(), # 步骤3: 无人机护卫有人机
                ActionAttackLogic(),     # 步骤4: 战斗 
                ActionPatrolFormation()  # 步骤5: 兜底
            ])
        ])
