...
def auto_engage_main(red_agent, blue_agent):
    ...
    while True:
        ...
        start_once = True
        red_agent.__init__(red_agent.side, red_agent.name)
        blue_agent.__init__(blue_agent.side, blue_agent.name)
        while not done[0]:
            # try:
        ...
        print(f"推演(第{env.current_run_time}轮)结束:{done[0]} 红方胜利:{done[1]} 蓝方胜利:{done[2]}")
        env.funTool.sim_control('edit')
        if env.run_times <= 0:
