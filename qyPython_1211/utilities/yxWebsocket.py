...
class YXWebSocketClient:
    ...
    def _on_close_wrapper(self, ws, close_status_code, close_msg):
        ...
        if close_status_code or close_msg:
            logger.debug(f"[{self.client_id}] 连接关闭: 状态码={close_status_code}, 消息={close_msg}")
        else:
            logger.debug(f"[{self.client_id}] 连接关闭")
    ...
    def _default_on_message(self, ws, message):
        """默认消息处理器"""
        logger.debug(f"[{self.client_id}] 收到消息: {message[:80]}{'...' if len(message) > 80 else ''}")
    ...
    def _default_on_close(self, ws, status, msg):
        logger.debug(f"[{self.client_id}] 连接关闭: {status} - {msg}")
    ...
    def close(self):
        ...
        logger.debug(f"[{self.client_id}] 客户端已关闭")
...
class YXWebSocketClientManager:
    ...
    def close_all(self):
        ...
        with self.lock:
            for url, client in self.clients.items():
                client.close()
            self.clients.clear()
        logger.debug("所有WebSocket客户端已关闭")
    ...
    def stop_all(self):
        ...
        logger.debug("所有WebSocket客户端已停止")