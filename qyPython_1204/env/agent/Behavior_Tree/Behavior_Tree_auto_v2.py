...
class ActionResetFrame(Action):
    ...
    def tick(self, agent) -> str:
        ...
        agent.missile_threats = {}        # 清空当帧导弹处理记录
        return NodeStatus.SUCCESS
...
class ActionEvadeMissiles(Action):
    ...
    def tick(self, agent) -> str:
        ...
        for missile in agent.enemy_missiles:
            missile_id = agent.make_entity_key(missile)
            if missile_id in missile_avoided_this_frame:
                continue
            ...
            for unit in threatened_units:
                ...
                lat_ref = unit.get('latitude', agent.center_lat)
                lon_off, lat_off = YxGeoUtils.km_to_lon_lat(lat_ref, 5, evade_dir)
                ...
                agent.missile_threats[missile_id] = unit['name']
                break
...
class ActionAttackLogic(Action):
    ...
    def tick(self, agent) -> str:
        ...
        for pair in dist_pairs:
            ...
            # 攻击条件检查
            # 1. 敌机未被过度攻击
            target_key = agent.make_entity_key(e)
            already_fired = e.get('is_fired_num', 0) + cur_round_shots.get(target_key, 0)
            ...
            if agent.try_fire_weapon(u):
                ...
                cur_round_shots[target_key] = cur_round_shots.get(target_key, 0) + 1
        ...
        for unit in agent.own_units:
            ...
            if closest:
                ...
                lon_off, lat_off = YxGeoUtils.km_to_lon_lat(closest['latitude'], optimal_dist/1000, direction)
...
class BTDemoAgent_v2(AutoAgentBase):
    def __init__(self, side, name):
        ...
        self.bt_root = Selector([
            # 分支 1: 开局部署
            Sequence([
                ConditionCheckInitialDeployment(), # 检查是否完成了初始部署
    ...
    def make_entity_key(self, entity):
        """根据实体可用字段生成稳定的 key"""
        if not isinstance(entity, dict):
            return id(entity)
        for key in ('target_id', 'track_id', 'target_name', 'name', 'id'):
